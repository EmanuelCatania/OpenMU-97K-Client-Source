...
extern "C" _declspec(dllexport) void _cdecl EntryProc()
{
	...
	// ProtectionManager ya fue inicializado en DllMain
	...
	gPacketManager.SetXorFilter(gProtect.m_MainInfo.XorFilter, sizeof(gProtect.m_MainInfo.XorFilter));
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
{
	switch (ul_reason_for_call)
	{
		case DLL_PROCESS_ATTACH:
		{
			...
			// Inicializar ProtectionManager con OpenMU al cargar el DLL
			InitializeProtectionManager();
		...
		{
			// Limpiar ProtectionManager cuando se descarga el DLL
			ShutdownProtectionManager();
			break;
		}
